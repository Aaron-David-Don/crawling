...
function extractUrlsFromCsv(filename, startIndex, callback) {
  const urls = [];
...
async function scrapeTextFromUrl(url) {
  try {
    // var client = new scrapingbee.ScrapingBeeClient("ZTFP3TWZFKRUANE28YWT6QT00QG4340214QHB96DOL40GHW9U7UKSPA4DWNQZWJSQ5HSHUJCPY55AGQ5");
    const scraperapiClient = require('scraperapi-sdk')('fac50afc0966e2dd9e45ea458551667f')
    ...
    let cleanedTextContent = textContent
      ...
      // Limit the content to 28,000 characters
      if (cleanedTextContent.length > 21000) {
        cleanedTextContent = cleanedTextContent.substring(0, 21000);
      }
  ...
  for (const url of urls) {
    ...
    try {
      ...
      try {
        ...
        fs.appendFileSync('compro_json2csv.csv', url)
...
const startIndex = parseInt(process.argv[2], 10) || 0; // Get start index from command line arguments

// Extract URLs from the CSV file and process them
extractUrlsFromCsv(csvFilename, startIndex, (urlsList) => {
